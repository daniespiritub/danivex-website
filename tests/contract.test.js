...
test('CONTRATO: visual presente (avatar/banner/outfit/pet)', () => {
  ...
  assert.equal(r.pet, 'Falco', 'nombre de ESPECIE (petId 1300000091), NO el apodo ni el id')
  assert.equal(r.petNickname, 'Palomita', 'el name de la API es el apodo del jugador')
  assert.equal(r.petSkinName, 'Blooming Falco', 'nombre del aspecto (skinId 1310000097)')
  assert.doesNotMatch(r.pet, /^\d+$/, 'NUNCA el id numerico como nombre')
